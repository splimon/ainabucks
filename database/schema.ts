...
export const usersTable = pgTable("users", {
  ...
  totalAinaBucksRedeemed: integer("total_aina_bucks_redeemed").notNull().default(0),
  currentAinaBucks: integer("current_aina_bucks").notNull().default(0), // earned - redeemed
  totalHoursVolunteered: decimal("total_hours_volunteered", { precision: 8, scale: 2 }).notNull().default("0"),
...
export const eventAttendanceTable = pgTable("event_attendance", {
  ...
  registrationId: uuid("registration_id").notNull().references(() => eventRegistrationsTable.id, { onDelete: "cascade" }),
...
export const ainaBucksTransactionsTable = pgTable("aina_bucks_transactions", {
  ...
  // Foreign Keys
  userId: uuid("user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  eventId: uuid("event_id").references(() => eventsTable.id, { onDelete: "set null" }), // null if not event-related
  attendanceId: uuid("attendance_id").references(() => eventAttendanceTable.id, { onDelete: "set null" }),
...
export type AinaBucksTransaction = typeof ainaBucksTransactionsTable.$inferSelect; // For reading transaction data
export type NewAinaBucksTransaction = typeof ainaBucksTransactionsTable.$inferInsert; // For inserting new transactions
